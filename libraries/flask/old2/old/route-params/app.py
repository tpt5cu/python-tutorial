...
"""A route only takes GET requests by default. This route only takes POST requests. It will return 405 if a GET request is sent."""
...
"""
When a route parameter is defined to be a "path" type, Flask will accept the URL parameter with "/" characters. Flask does not check to see whether or
not the path exists on the server filesystem. If there are subsequent route parameters after a "path" parameter, Flask will smartly parse the incoming
URL so that each parameter is filled with a value. If every parameter cannot be filled with a value, a 404 is returned because the URL didn't match
the route.
"""
