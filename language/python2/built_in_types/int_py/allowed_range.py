...
"""
The plain int type is unbounded in Python 3, since in Python 3 an int is still named 'int' but it is really a long.
In Python, the range of a numeric value is only limited by the amount of memory on my machine, not by the word size of 64 bits. Therefore, integers
cannot overflow/underflow in pure Python because Python integers have arbitrary precision.
