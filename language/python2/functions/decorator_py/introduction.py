...
def repeat(num_times): # Use this to make a closure
    def decorator_repeat(func):
        def wrapper_repeat(*args, **kwargs):
            ...
            return value # wrapper_repeat returns a value ONCE. It also happens to use the wrapped function
        return wrapper_repeat # decorator_repeat returns a function. When THAT returned function is invoked, it will return a value once (but in this case it will also print x times)
    return decorator_repeat # repeat returns a function. When THAT function is invoked, it EXPECTS a function as an argument AND it returns a function.
