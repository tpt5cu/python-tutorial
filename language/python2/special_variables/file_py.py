...
this file name. For example, __file__ could be:
    - file_py.py
    - special_variables/file_py.py
    _ python2/special_variables/file_py.py
depending on where the $ python $ command was run from.
...
interpreter, unless the script directory isn't available, in which case path[0] is an empty string.
    - The script directory isn't available if this file is run as a module (i.e. with the -m flag) because python imports the specified file for me.
        - Thus, running this file as a module doesn't put the absolute path of this file's directory into sys.path, which is why __file__ will then 
        be an absolute path.
