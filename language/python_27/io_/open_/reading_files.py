...
def read_lines():
    '''
    readlines() uses readline() to insert all of the lines into a list! This isn't efficient for really large files, but it's convenient. The
    counterpart to readlines() is writelines().
    - readlines() is different from read() in that read() returns a string instead of a list
    '''
...
def use_iterator():
    ...
    Since next() uses a hidden read-ahead buffer for efficiencey, combining next() (or using a for-loop) with other methods like readline() doesn't
    work correctly. However, seek() can be used with an absolute position will flush the read-ahead buffer.
    '''
