...
'''
When a call to f.write() is made, the string is actually BUFFERRED to the Python process's own internal IOBuffer. Nothing is immediately written to
the file. Only once flush() is called is the buffer written to the file. The buffer is written to the file when one of the following happens: 1)
f.flush() is explicitly called 2) f.close() is called (which implicitly happens at the end of the with statement) or 3) the buffer fills up
...
- It's actually even more complicated. fsync() moves the data from the OS buffer to the drive. The drive ITSELF won't physically write the data until
  later.
'''
