...
def io_open_locking():
    '''
    The standard open() function and io.open() function can both (supposedly) be locked with fcntl.flock (or another lock type). fcntl operates on a
    file descriptor (an integer) or an object that implements a fileno() method. Both the Python <type 'file'> and <type '_io.TextIOWrapper'>
    implement a fileno() method 
    '''
